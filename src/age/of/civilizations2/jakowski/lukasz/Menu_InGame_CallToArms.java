...
extends SliderMenu {
    ...
    protected Menu_InGame_CallToArms(int onCivID) {
        ArrayList<MenuElement> menuElements = new ArrayList<MenuElement>();
        this.iOnCivID = onCivID;
        this.warAgainstCivID = this.warAgainstCivID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Diplomacy_CallToArms(Images.diplo_rivals, CFG.langManager.get("RequestHelpInTheWarAgainst") + ": ", onCivID, 2, tY, CFG.BUTTON_WIDTH * 2, 0){
        ...
        if (alliesToCall.size() > 0) {
            for (int i = 0; i < alliesToCall.size(); ++i) {
                menuElements.add(new Buton_Diplomacy_CallAllies(i, (int)alliesToCall.get(i), onCivID, 2, tY, CFG.BUTTON_WIDTH * 2){
