...
class Menu_InGame_Loans
extends SliderMenu {
    private int iOnCivID = -1;

    protected Menu_InGame_Loans() {
    ...
    protected Menu_InGame_Loans(int onCivID) {
        ...
        if (CFG.game.getCiv(onCivID).getLoansSize() > 0) {
            for (int i = 0; i < CFG.game.getCiv(onCivID).getLoansSize(); ++i) {
                menuElements.add(new Buton_Diplomacy_Loan(i, onCivID, CFG.game.getCiv((int)onCivID).getLoan((int)i).iGoldPerTurn, CFG.game.getCiv((int)onCivID).getLoan((int)i).iGoldPerTurn * CFG.game.getCiv((int)onCivID).getLoan((int)i).iTurnsLeft, CFG.game.getCiv((int)onCivID).getLoan((int)i).iTurnsLeft, 2, tY, CFG.BUTTON_WIDTH * 2){
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Loans"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                ImageManager.getImage(Images.diplo_loan).draw(oSB, nPosX + (int)((float)nWidth - ((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_loan).getWidth() + (float)CFG.PADDING)) / 2 + iTranslateX, Menu_InGame_Loans.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.diplo_loan).getHeight() / 2);
                CFG.fontMain.getData().setScale(0.8f);
                CFG.drawText(oSB, this.getText(), nPosX + (int)((float)nWidth - ((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_loan).getWidth() + (float)CFG.PADDING)) / 2 + ImageManager.getImage(Images.diplo_loan).getWidth() + CFG.PADDING + iTranslateX, 2 + nPosY - this.getHeight() + (int)((float)this.getHeight() - (float)this.getTextHeight() * 0.8f) / 2, Color.WHITE);
