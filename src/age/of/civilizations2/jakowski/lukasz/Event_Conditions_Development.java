...
class Event_Conditions_Development extends Event_Conditions {
   ...
   protected boolean outCondition() {
      try {
         for(int i = 0; i < this.getProvinces().size(); ++i) {
            if (CFG.game.getProvince(this.getProvinces().get(i)).getDevelopmentLevel() < (float)this.getValue() / 100.0F) {
               return false;
            }
         }
   ...
   protected String getConditionText() {
      try {
         return CFG.langManager.get("DevelopmentLevel") + " >= " + (float)this.getValue() / 100.0F;
      } catch (IndexOutOfBoundsException var2) {
