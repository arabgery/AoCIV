...
extends SliderMenu {
    ...
    protected Menu_Editor_Unions_List() {
        ...
        for (int i = 0; i < CFG.unionsManager.unions.lUnions.size(); ++i) {
            ...
            for (int j = 0; j < CFG.unionsManager.unions.lUnions.get((int)i).lCivsTags.size(); ++j) {
                tTags = tTags + "" + CFG.langManager.getCiv(CFG.unionsManager.unions.lUnions.get((int)i).lCivsTags.get(j)) + (CFG.unionsManager.unions.lUnions.get((int)i).lCivsTags.size() - 1 == j ? "" : "-");
            }
            menuElements.add(new Button_Menu(CFG.langManager.get("Civilization") + ": " + CFG.langManager.getCiv(CFG.unionsManager.unions.lUnions.get((int)i).lCreateCivTag) + tTags, (int)(50.0f * CFG.GUI_SCALE), 0, CFG.PADDING * (i + 2) + CFG.BUTTON_HEIGHT * (i + 1), CFG.GAME_WIDTH - CFG.BUTTON_WIDTH - CFG.BUTTON_WIDTH / 2, CFG.BUTTON_HEIGHT, true));
            menuElements.add(new Button_Menu_Remove(CFG.GAME_WIDTH - CFG.BUTTON_WIDTH - CFG.BUTTON_WIDTH / 2, CFG.PADDING * (i + 2) + CFG.BUTTON_HEIGHT * (i + 1), CFG.BUTTON_WIDTH + CFG.BUTTON_WIDTH / 2, CFG.BUTTON_HEIGHT, true));
    ...
    protected void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        try {
            for (int i = tempRandomButton; i < this.getMenuElementsSize(); i += 2) {
                ...
                this.lFlags.get(this.getFlagID((i - tempRandomButton) / 2)).draw(oSB, this.getMenuElement(i).getPosX() + this.getMenuElement(i).getTextPos() / 2 - CFG.CIV_FLAG_WIDTH / 2 + iTranslateX, this.getMenuElement(i).getPosY() + this.getMenuPosY() + this.getMenuElement(i).getHeight() / 2 - this.lFlags.get(this.getFlagID((i - tempRandomButton) / 2)).getHeight() - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY, CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
