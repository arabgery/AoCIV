...
class Menu_InGame_Build_Workshop
extends SliderMenu {
    private int iProvinceID = -1;

    protected Menu_InGame_Build_Workshop() {
        ...
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Workshop"), CFG.BUTTON_HEIGHT * 3 / 5, true, true), CFG.GAME_WIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING + tempMenuPosY > CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 ? Math.max(CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT + CFG.PADDING * 2) * 6) : ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, menuElements, false, true);
        this.updateLanguage();
    }

    protected Menu_InGame_Build_Workshop(int nProvinceID) {
        ArrayList<MenuElement> menuElements = new ArrayList<MenuElement>();
        this.iProvinceID = nProvinceID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Build_Building(CFG.langManager.get("BuildWorkshopIn") + ": ", CFG.game.getProvince(this.iProvinceID).getName().length() > 0 ? CFG.game.getProvince(this.iProvinceID).getName() : CFG.langManager.get("Province"), Images.b_workshop, BuildingsManager.getWorkshop_BuildCost(CFG.game.getProvince(this.iProvinceID).getLevelOfWorkshop() + 1, this.iProvinceID), BuildingsManager.getWorkshop_BuildMovementCost(CFG.game.getProvince(this.iProvinceID).getLevelOfWorkshop() + 1), 0, tY, CFG.BUTTON_WIDTH * 2){
            ...
            protected void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("+" + (int)(BuildingsManager.getWorkshop_IncomeProduction(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1) * 100.0f) + "%", CFG.COLOR_TEXT_MODIFIER_POSITIVE));
                nElements.add(new MenuElement_Hover_v2_Element2(nData));
                nData.clear();
                this.menuElementHover = new MenuElement_Hover_v2(nElements);
            }
        });
        ((MenuElement)menuElements.get(menuElements.size() - 1)).setMin((int)(BuildingsManager.getWorkshop_TechLevel(CFG.game.getProvince(this.iProvinceID).getLevelOfWorkshop() + 1) * 100.0f));
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Construct"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getName().length() > 0 ? CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getName() : CFG.langManager.get("Province")));
                nData.add(new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getCivID(), CFG.PADDING, 0));
                nElements.add(new MenuElement_Hover_v2_Element2(nData));
                nData.clear();
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + BuildingsManager.getWorkshop_BuildCost(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1, Menu_InGame_Build_Workshop.this.iProvinceID), CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() >= (long)BuildingsManager.getWorkshop_BuildCost(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1, Menu_InGame_Build_Workshop.this.iProvinceID) ? CFG.COLOR_INGAME_GOLD : CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + (float)BuildingsManager.getWorkshop_BuildMovementCost(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1) / 10.0f, CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMovePoints() >= BuildingsManager.getWorkshop_BuildMovementCost(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1) ? CFG.COLOR_INGAME_MOVEMENT : CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
                nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.top_movement_points, CFG.PADDING, 0));
                nElements.add(new MenuElement_Hover_v2_Element2(nData));
                nData.clear();
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("ConstructionWillTakeXurns", BuildingsManager.getWorkshop_Construction(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1))));
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + (float)((int)(BuildingsManager.getWorkshop_TechLevel(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1) * 100.0f)) / 100.0f, CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getTechnologyLevel() >= BuildingsManager.getWorkshop_TechLevel(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1) ? CFG.COLOR_TEXT_TECHNOLOGY : CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
                nData.add(new MenuElement_Hover_v2_Element_Type_Image(Images.technology, CFG.PADDING, 0));
                nData.add(new MenuElement_Hover_v2_Element_Type_Image(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getTechnologyLevel() >= BuildingsManager.getWorkshop_TechLevel(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1) ? Images.icon_check_true : Images.icon_check_false, CFG.PADDING, 0));
                nElements.add(new MenuElement_Hover_v2_Element2(nData));
                nData.clear();
                this.menuElementHover = new MenuElement_Hover_v2(nElements);
            }
            ...
            protected boolean getClickable() {
                return CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMovePoints() >= BuildingsManager.getWorkshop_BuildMovementCost(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1) && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() >= (long)BuildingsManager.getWorkshop_BuildCost(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1, Menu_InGame_Build_Workshop.this.iProvinceID) && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getTechnologyLevel() >= BuildingsManager.getWorkshop_TechLevel(CFG.game.getProvince(Menu_InGame_Build_Workshop.this.iProvinceID).getLevelOfWorkshop() + 1);
            }
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get(BuildingsManager.getWorkshop_Name(CFG.game.getProvince(this.iProvinceID).getLevelOfWorkshop() + 1)), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getFlag().draw(oSB, Menu_InGame_Build_Workshop.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_Build_Workshop.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getFlag().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                ImageManager.getImage(Images.flag_rect).draw(oSB, Menu_InGame_Build_Workshop.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_Build_Workshop.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
                CFG.fontMain.getData().setScale(0.8f);
