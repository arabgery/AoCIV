...
extends SliderMenu {
    ...
    protected Menu_InGame_CreateAVassal_SelectCiv_List() {
        ...
        if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
            ...
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0 || CFG.isInTheGame_Or_IsFormableCiv(tagsSPLITED[i])) continue;
                lTempNames.add(CFG.langManager.getCiv(tagsSPLITED[i]));
                lTempTags.add(tagsSPLITED[i]);
            }
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED_ED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0 || CFG.isInTheGame_Or_IsFormableCiv(tagsSPLITED_ED[i])) continue;
            ...
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.isInTheGame_Or_IsFormableCiv(tagsSPLITED_ED[i])) continue;
                lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[i]));
                lTempTags.add(tagsSPLITED_ED[i]);
            }
            CFG.menuManager.menuCreateVassal_SelectCiv_UpdateTitle(lTempNames.size());
            ...
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED_ED[i]).charAt(0) != CFG.chosen_AlphabetCharachter.charAt(0) || CFG.isInTheGame_Or_IsFormableCiv(tagsSPLITED_ED[i])) continue;
                lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[i]));
                lTempTags.add(tagsSPLITED_ED[i]);
            }
            CFG.menuManager.menuCreateVassal_SelectCiv_UpdateTitle(lTempNames.size());
