...
import age.of.civilizations2.jakowski.lukasz.Button_Build_TransferControl;
...
class Menu_InGame_TransferControl
extends SliderMenu {
    ...
    protected Menu_InGame_TransferControl(int nProvinceID) {
        ...
        if (tWarID >= 0) {
            ...
            if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() > 0) {
                for (i = 0; i < CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilizationsSize(); ++i) {
                    if (CFG.game.getCiv(CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilization(i)).getNumOfProvinces() <= 0 || CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilization(i) == CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() || !CFG.game.getWar(tWarID).getIsInDefenders(CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilization(i)) && !CFG.game.getWar(tWarID).getIsAggressor(CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilization(i))) continue;
                    alliesToTransfer.add(CFG.game.getAlliance(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID()).getCivilization(i));
                }
            }
            for (i = 1; i < CFG.game.getCivsSize(); ++i) {
                if (i == CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() || CFG.game.getCiv(i).getPuppetOfCivID() != CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID() != i || CFG.game.getCiv(i).getNumOfProvinces() <= 0 || !CFG.game.getWar(tWarID).getIsInDefenders(i) && !CFG.game.getWar(tWarID).getIsAggressor(i)) continue;
            ...
            if (alliesToTransfer.size() == 0) {
                ...
                for (i = 0; i < alliesToTransfer.size(); ++i) {
                    menuElements.add(new Buton_Diplomacy_CallAllies(i, (int)((Integer)alliesToTransfer.get(i)), CFG.game.getProvince(this.iProvinceID).getTrueOwnerOfProvince(), 2, tY, CFG.BUTTON_WIDTH * 2){
            ...
            menuElements.add(new Buton_Diplomacy_CallAllies(0, CFG.game.getProvince(this.iProvinceID).getTrueOwnerOfProvince(), CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), 2, tY, CFG.BUTTON_WIDTH * 2){
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, canSend){
            ...
            protected void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(Menu_InGame_TransferControl.this.iProvinceID).getCivID(), CFG.PADDING, 0));
        ...
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("TransferControl"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getFlag().draw(oSB, Menu_InGame_TransferControl.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_TransferControl.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getFlag().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                ImageManager.getImage(Images.flag_rect).draw(oSB, Menu_InGame_TransferControl.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_TransferControl.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
                CFG.fontMain.getData().setScale(0.8f);
