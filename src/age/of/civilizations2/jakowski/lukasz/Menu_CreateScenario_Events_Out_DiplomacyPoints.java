...
class Menu_CreateScenario_Events_Out_DiplomacyPoints
extends SliderMenu {
    protected Menu_CreateScenario_Events_Out_DiplomacyPoints() {
        ...
        menuElements.add(new Slider_BG(0, tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING * 2, CFG.GAME_WIDTH, CFG.BUTTON_HEIGHT - CFG.PADDING * 2, -100, 100, CFG.eventsManager.lCreateScenario_Event.lDecisions.get((int)CFG.eventsManager.iCreateEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.iCreateEvent_EditConditionID).getValue()){
            ...
            protected String getDrawText() {
                return this.getText() + (this.getCurrent() > 0 ? "+" : "") + (float)this.getCurrent() / 100.0f;
            }
        });
