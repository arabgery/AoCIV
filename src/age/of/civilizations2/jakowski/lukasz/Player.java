...
class Player {
   ...
   protected final float buildPlayerScore() {
      ...
      for(int i = 0; i < CFG.game.getCiv(this.getCivID()).getNumOfProvinces(); ++i) {
         out += 2.45F
            * (float)CFG.game.getProvince(CFG.game.getCiv(this.getCivID()).getProvinceID(i)).getPopulationData().getPopulation()
            / (float)CFG.game.getGameScenarios().getScenario_StartingPopulation();
         out += 2.25F
            * (float)CFG.game.getProvince(CFG.game.getCiv(this.getCivID()).getProvinceID(i)).getEconomy()
            / (float)CFG.game.getGameScenarios().getScenario_StartingEconomy();
      }

      return out + 0.075F * (float)CFG.game.getCiv(this.getCivID()).civGameData.iNumOfConqueredProvinces;
   }
   ...
   protected final void buildMetProvincesAndCivs() {
      ...
      if (CFG.game.getCiv(this.getCivID()).getIsPartOfHolyRomanEmpire()) {
         for(int i = 0; i < CFG.holyRomanEmpire_Manager.getHRE().getPrincesSize(); ++i) {
            for(int j = 0; j < CFG.game.getCiv(CFG.holyRomanEmpire_Manager.getHRE().getPrince(i)).getNumOfProvinces(); ++j) {
      ...
      for(int i = 0; i < CFG.game.getProvincesSize(); ++i) {
         if (!this.getMetProvince(i)) {
            ...
            for(int j = 0; j < CFG.game.getCiv(this.getCivID()).getNumOfProvinces(); ++j) {
               if (CFG.game_NextTurnUpdate.getDistanceFromAToB_PercOfMax(CFG.game.getCiv(this.getCivID()).getProvinceID(j), i)
                     * (
                        CFG.game.getProvince(CFG.game.getCiv(this.getCivID()).getProvinceID(j)).getContinent() == CFG.game.getProvince(i).getContinent()
                           ? 0.715F
                           : 1.0F
                     )
                  < (
                                 ...
                                 1.0F
                                    - Math.min(
                                       (float)Math.abs(
                                          ...
                                          / ((float)CFG.map.getMapBG().getHeight() / 10.0F),
                                       1.0F
                                    )
                              )
                     )
