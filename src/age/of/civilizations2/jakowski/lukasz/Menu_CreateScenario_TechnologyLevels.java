...
class Menu_CreateScenario_TechnologyLevels
extends Menu_CreateScenario {
    private String sTopText;
    private int iStepWidth;

    protected Menu_CreateScenario_TechnologyLevels() {
        ...
        menuElements.add(new Slider(CFG.BUTTON_WIDTH + CFG.PADDING * 2, CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING, CFG.GAME_WIDTH - (CFG.BUTTON_WIDTH + CFG.PADDING * 2) * 2, CFG.BUTTON_HEIGHT, 5, 100, 100){

            @Override
            protected String getDrawText() {
                return CFG.game.getCiv(CFG.iCreateScenario_AssignProvinces_Civ).getCivName() + this.getText() + (float)this.getCurrent() / 100.0f;
            }
    ...
    protected final void actionElement(int iID) {
        switch (iID) {
            ...
            case 3: {
                ...
                if (CFG.iCreateScenario_AssignProvinces_Civ > 0) {
                    CFG.game.getCiv(CFG.iCreateScenario_AssignProvinces_Civ).setTechnologyLevel((float)this.getMenuElement(iID + 1).getCurrent() / 100.0f);
                    CFG.game.getProvince(CFG.game.getCiv(CFG.iCreateScenario_AssignProvinces_Civ).getCapitalProvinceID()).getArmy_Obj(0).updateArmyWidth(CFG.game.getCiv(CFG.iCreateScenario_AssignProvinces_Civ).getTechnologyLevel());
                }
                return;
            }
            case 4: {
                if (CFG.iCreateScenario_AssignProvinces_Civ > 0) {
                    CFG.game.getCiv(CFG.iCreateScenario_AssignProvinces_Civ).setTechnologyLevel((float)this.getMenuElement(iID).getCurrent() / 100.0f);
                    CFG.game.getProvince(CFG.game.getCiv(CFG.iCreateScenario_AssignProvinces_Civ).getCapitalProvinceID()).getArmy_Obj(0).updateArmyWidth(CFG.game.getCiv(CFG.iCreateScenario_AssignProvinces_Civ).getTechnologyLevel());
                }
                return;
            }
