...
import age.of.civilizations2.jakowski.lukasz.Buton_Diplomacy_Loan;
...
class Menu_InGame_RepayLoan
extends SliderMenu {
    private int iOnCivID = -1;
    private int iLoanValue = 0;
    private int iLoanID;

    protected Menu_InGame_RepayLoan() {
    ...
    protected Menu_InGame_RepayLoan(int onCivID, int iLoanID) {
        ...
        if (iLoanID >= 0 && iLoanID < CFG.game.getCiv(onCivID).getLoansSize()) {
            this.iLoanValue = CFG.game.getCiv((int)onCivID).getLoan((int)iLoanID).iGoldPerTurn * CFG.game.getCiv((int)onCivID).getLoan((int)iLoanID).iTurnsLeft;
            menuElements.add(new Buton_Diplomacy_Loan(0, onCivID, CFG.game.getCiv((int)onCivID).getLoan((int)iLoanID).iGoldPerTurn, CFG.game.getCiv((int)onCivID).getLoan((int)iLoanID).iGoldPerTurn * CFG.game.getCiv((int)onCivID).getLoan((int)iLoanID).iTurnsLeft, CFG.game.getCiv((int)onCivID).getLoan((int)iLoanID).iTurnsLeft, 2, tY, CFG.BUTTON_WIDTH * 2){
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Repay"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                ImageManager.getImage(Images.diplo_loan2).draw(oSB, this.getPosX() + this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_loan2).getWidth() + (float)CFG.PADDING) / 2.0f) + iTranslateX, this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_loan2).getHeight() / 2 + iTranslateY);
                CFG.fontMain.getData().setScale(0.8f);
                CFG.drawText(oSB, this.getText(), this.getPosX() + (this.getTextPos() < 0 ? this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_loan2).getWidth() + (float)CFG.PADDING) / 2.0f) + ImageManager.getImage(Images.diplo_loan2).getWidth() + CFG.PADDING : this.getTextPos()) + iTranslateX, this.getPosY() + this.getHeight() / 2 - (int)((float)this.getTextHeight() * 0.8f / 2.0f) + iTranslateY, this.getColor(isActive));
                CFG.fontMain.getData().setScale(1.0f);
            }

            @Override
            protected boolean getClickable() {
                return CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() >= (long)Menu_InGame_RepayLoan.this.iLoanValue;
            }
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Loan"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                ImageManager.getImage(Images.diplo_loan2).draw(oSB, nPosX + (int)((float)nWidth - ((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_loan2).getWidth() + (float)CFG.PADDING)) / 2 + iTranslateX, Menu_InGame_RepayLoan.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.diplo_loan2).getHeight() / 2);
                CFG.fontMain.getData().setScale(0.8f);
                CFG.drawText(oSB, this.getText(), nPosX + (int)((float)nWidth - ((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_loan2).getWidth() + (float)CFG.PADDING)) / 2 + ImageManager.getImage(Images.diplo_loan2).getWidth() + CFG.PADDING + iTranslateX, 2 + nPosY - this.getHeight() + (int)((float)this.getHeight() - (float)this.getTextHeight() * 0.8f) / 2, Color.WHITE);
                CFG.fontMain.getData().setScale(1.0f);
            }
