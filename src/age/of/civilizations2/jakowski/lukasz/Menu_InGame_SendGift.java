...
class Menu_InGame_SendGift
extends SliderMenu {
    private int iOnCivID = -1;

    protected Menu_InGame_SendGift() {
    ...
    protected Menu_InGame_SendGift(int onCivID) {
        ...
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        menuElements.add(new Slider_FlagAction_Gold(CFG.langManager.get("SelectTheAmountOfMoneyToSend"), CFG.PADDING * 2, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4, 0, CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() > 0L ? (int)((float)CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() * 0.25f) : 0, 0, 0.65f){
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                ImageManager.getImage(Images.diplo_gift).draw(oSB, this.getPosX() + this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_gift).getWidth() + (float)CFG.PADDING) / 2.0f) + iTranslateX, this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_gift).getHeight() / 2 + iTranslateY);
                CFG.fontMain.getData().setScale(0.8f);
                CFG.drawText(oSB, this.getText(), this.getPosX() + (this.getTextPos() < 0 ? this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_gift).getWidth() + (float)CFG.PADDING) / 2.0f) + ImageManager.getImage(Images.diplo_gift).getWidth() + CFG.PADDING : this.getTextPos()) + iTranslateX, this.getPosY() + this.getHeight() / 2 - (int)((float)this.getTextHeight() * 0.8f / 2.0f) + iTranslateY, this.getColor(isActive));
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("SendGift"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                ImageManager.getImage(Images.top_gold).draw(oSB, nPosX + (int)((float)nWidth - ((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.top_gold).getWidth() + (float)CFG.PADDING)) / 2 + iTranslateX, Menu_InGame_SendGift.this.getPosY() - this.getHeight() / 2 - ImageManager.getImage(Images.top_gold).getHeight() / 2);
                CFG.fontMain.getData().setScale(0.8f);
    ...
    protected final void actionElement(int iID) {
        if (iID == this.getMenuElementsSize() - 1) {
            ...
            DiplomacyManager.sendGift(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent());
