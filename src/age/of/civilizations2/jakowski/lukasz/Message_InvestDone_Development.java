...
public class Message_InvestDone_Development extends Message {
   protected Message_InvestDone_Development(int fromCivID, int iValue) {
   ...
   protected MenuElement_Hover_v2 getHover() {
      ...
      if (CFG.game.getCiv(this.iFromCivID).getTechnologyLevel() - 0.01F <= CFG.game.getProvince(this.iValue).getDevelopmentLevel()) {
         ...
         nData.add(
            new MenuElement_Hover_v2_Element_Type_Text(
               "" + (float)((int)(CFG.game.getCiv(this.iFromCivID).getTechnologyLevel() * 100.0F)) / 100.0F, CFG.COLOR_TEXT_TECHNOLOGY
            )
         );
