...
class Menu_Editor_Unions_AddCiv_List
extends SliderMenu {
    ...
    protected Menu_Editor_Unions_AddCiv_List() {
        ...
        if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
            ...
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED_ED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0) continue;
                ...
                menuElements.add(new Button_Menu_Classic_Wiki(tID++, CFG.GAME_WIDTH - (CFG.BUTTON_WIDTH + CFG.BUTTON_WIDTH / 2), CFG.BUTTON_HEIGHT * nPosY + CFG.PADDING * (nPosY + 1), CFG.BUTTON_WIDTH + CFG.BUTTON_WIDTH / 2, CFG.BUTTON_HEIGHT, true){
                    ...
                    protected void buildElementHover() {
                        try {
                            ...
                            nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.getWikiInormationsLink_Clear((String)Menu_Editor_Unions_AddCiv_List.this.lCivsTags.get(this.getTextPos())), CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
            ...
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED_ED[i]).charAt(0) != CFG.chosen_AlphabetCharachter.charAt(0)) continue;
                lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[i]));
                lTempTags.add(tagsSPLITED_ED[i]);
            }
            int nPosY = 0;
    ...
    protected void updateMenuElements_IsInView() {
        ...
        for (int i = tempRandomButton = 0; i < this.getMenuElementsSize(); i += 2) {
    ...
    protected void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        try {
            for (int i = tempRandomButton; i < this.getMenuElementsSize(); i += 2) {
                ...
                this.lFlags.get(this.getFlagID((i - tempRandomButton) / 2)).draw(oSB, this.getMenuElement(i).getPosX() + this.getMenuElement(i).getTextPos() / 2 - CFG.CIV_FLAG_WIDTH / 2 + iTranslateX, this.getMenuElement(i).getPosY() + this.getMenuPosY() + this.getMenuElement(i).getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY);
    ...
    protected final void actionElement(int iID) {
        if (CFG.MANAGE_DIPLOMACY_ADD_NEW_PACT_CIV1 == -2) {
            ...
            try {
                CFG.unionsManager.createUnion_Data.lCivsTags.set(CFG.MANAGE_DIPLOMACY_ADD_NEW_PACT_CIV1, CFG.ideologiesManager.getRealTag(this.lCivsTags.get(iID / 2)));
            }
            catch (IndexOutOfBoundsException indexOutOfBoundsException) {}
        }
        this.onBackPressed();
