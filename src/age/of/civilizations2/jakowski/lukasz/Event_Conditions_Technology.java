...
class Event_Conditions_Technology extends Event_Conditions {
   ...
   protected boolean outCondition() {
      try {
         return CFG.game.getCiv(this.getCivID()).getTechnologyLevel() >= (float)this.getValue() / 100.0F;
   ...
   protected String getConditionText() {
      try {
         return CFG.langManager.get("TechnologyLevel") + " >= " + (float)this.getValue() / 100.0F + ", " + CFG.game.getCiv(this.getCivID()).getCivName();
      } catch (IndexOutOfBoundsException var2) {
