...
class Menu_InGame_Army
extends SliderMenu {
    protected static int iSort = 1;

    protected final int getRecruitedArmy(int nCivID) {
        return CFG.SPECTATOR_MODE || CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == CFG.game.getSortedCivsAZ(nCivID - 1) || CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() > 0 && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() == CFG.game.getCiv(CFG.game.getSortedCivsAZ(nCivID - 1)).getAllianceID() || CFG.game.getCiv(CFG.game.getSortedCivsAZ(nCivID - 1)).getPuppetOfCivID() == CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() ? CFG.game.getCiv(CFG.game.getSortedCivsAZ(nCivID - 1)).getNumOfUnits() : -1;
    }

    protected final int getRecruitedArmy_2(int nCivID) {
        return CFG.SPECTATOR_MODE || CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() == nCivID || CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() > 0 && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getAllianceID() == CFG.game.getCiv(nCivID).getAllianceID() || CFG.game.getCiv(nCivID).getPuppetOfCivID() == CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() ? CFG.game.getCiv(nCivID).getNumOfUnits() : -1;
    }

    protected Menu_InGame_Army(int tInit) {
    ...
    protected Menu_InGame_Army() {
        ...
        menuElements.add(new Button_Statistics_Title(CFG.langManager.get("Army"), CFG.PADDING, CFG.PADDING * 2 + CFG.BUTTON_WIDTH * 5, 0, CFG.BUTTON_WIDTH, tElemHeight){
