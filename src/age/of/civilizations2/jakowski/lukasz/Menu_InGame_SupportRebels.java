...
extends SliderMenu {
    ...
    protected Menu_InGame_SupportRebels(int onCivID, int iRebelsID) {
        ...
        if (supportData.lMovements.size() > 0) {
            ...
            menuElements.add(new Slider_FlagAction_Gold(CFG.langManager.get("SelectTheAmountOfMoneyToSend"), CFG.PADDING * 2, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4, 0, CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() > 0L ? (int)CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() : 0, 0, 0.65f){
    ...
    protected final void actionElement(int iID) {
        if (iID == this.getMenuElementsSize() - 1) {
            ...
            if (DiplomacyManager.supportRebels(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), iOnCivID, iSelectedCivID, this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent())) {
        ...
        this.getMenuElement(this.getMenuElementsSize() - 3).setMax(Math.max(0, Math.min((int)CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney(), DiplomacyManager.supportRebels_MaxGold(rebelsProvinces))));
