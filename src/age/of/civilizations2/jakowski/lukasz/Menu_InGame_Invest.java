...
import age.of.civilizations2.jakowski.lukasz.Button_Build_Invest;
...
class Menu_InGame_Invest
extends SliderMenu {
    protected static int iProvinceID = -1;

    protected Menu_InGame_Invest() {
    ...
    protected Menu_InGame_Invest(int nProvinceID) {
        ArrayList<MenuElement> menuElements = new ArrayList<MenuElement>();
        iProvinceID = nProvinceID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Build_Invest(CFG.langManager.get("InvestIn") + ": ", CFG.game.getProvince(iProvinceID).getName().length() > 0 ? CFG.game.getProvince(iProvinceID).getName() : CFG.langManager.get("Province"), Images.economy, 0, 12, 0, tY, CFG.BUTTON_WIDTH * 2){
        ...
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        menuElements.add(new Slider_FlagAction_Gold(CFG.langManager.get("Gold"), CFG.PADDING * 2, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4, 0, DiplomacyManager.invest_MaxEconomy_Gold(nProvinceID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()), 0, 0.65f){
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.getNumberWithSpaces("" + CFG.game.getProvince(iProvinceID).getEconomy()), CFG.COLOR_TEXT_ECONOMY));
        ...
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Invest"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getFlag().draw(oSB, Menu_InGame_Invest.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_Invest.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getFlag().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                ImageManager.getImage(Images.flag_rect).draw(oSB, Menu_InGame_Invest.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_Invest.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
                CFG.fontMain.getData().setScale(0.8f);
        ...
        if (iID == this.getMenuElementsSize() - 1) {
            if (DiplomacyManager.invest(iProvinceID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.getMenuElement(1).getCurrent())) {
                ...
                if (CFG.viewsManager.getActiveViewID() == ViewsManager.VIEW_ECONOMY_MODE) {
                    CFG.game.getProvince((int)Menu_InGame_Invest.iProvinceID).viewBool = true;
