...
public class Graph_Vertical_Data_Value_PopulationByProvinces extends Graph_Vertical_Data_Value {
   protected Graph_Vertical_Data_Value_PopulationByProvinces(int iValue, int iColorDataID) {
   ...
   protected final Color getColor(float fAlpha) {
      try {
         return new Color(
            (float)CFG.game.getCiv(CFG.game.getProvince(this.iColorDataID).getCivID()).getR() / 255.0F,
            (float)CFG.game.getCiv(CFG.game.getProvince(this.iColorDataID).getCivID()).getG() / 255.0F,
            (float)CFG.game.getCiv(CFG.game.getProvince(this.iColorDataID).getCivID()).getB() / 255.0F,
            fAlpha
         );
