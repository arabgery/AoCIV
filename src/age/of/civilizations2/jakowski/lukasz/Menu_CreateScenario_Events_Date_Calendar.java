...
class Menu_CreateScenario_Events_Date_Calendar
extends SliderMenu {
    protected Menu_CreateScenario_Events_Date_Calendar() {
        ...
        menuElements.add(new Button_ArrowLeft(0, CFG.PADDING, CFG.BUTTON_HEIGHT, CFG.BUTTON_HEIGHT * 2 / 3){
            ...
            protected void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(Game_Calendar.getMonthName(CFG.eventsManager.iCreateEvent_Month - 1), CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
        ...
        menuElements.add(new Text(Game_Calendar.getMonthName(CFG.eventsManager.iCreateEvent_Month), -1, CFG.BUTTON_HEIGHT, CFG.PADDING, tempW - CFG.BUTTON_HEIGHT * 2, CFG.BUTTON_HEIGHT * 2 / 3){
        ...
        for (int i = 0; i < Game_Calendar.getNumOfDaysInMonth(CFG.eventsManager.iCreateEvent_Month); ++i) {
            menuElements.add(new Button_CalendarDay(i + 1, CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 2 / 3 * tX + CFG.PADDING * tX++, tH){
                ...
                protected Color getColor(boolean isActive) {
                    return isActive || CFG.eventsManager.iCreateEvent_Day == this.getCurrent() ? CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE : (this.getClickable() ? new Color(0.38f, 0.38f, 0.38f, 1.0f) : new Color(0.49f, 0.49f, 0.49f, 0.5f));
                }
            });
            if (tX != 7) continue;
