...
class Menu_InGame_SendInsult
extends SliderMenu {
    private int iOnCivID = -1;

    protected Menu_InGame_SendInsult() {
    ...
    protected Menu_InGame_SendInsult(int onCivID) {
        ...
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        menuElements.add(new Slider_FlagAction_Date(CFG.langManager.get("SuspendDiplomaticRelations"), CFG.PADDING * 2, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4, 15, 50, 43, 0.65f){
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("SendAnInsult"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                ImageManager.getImage(Images.diplo_relations_dec).draw(oSB, this.getPosX() + this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_relations_dec).getWidth() + (float)CFG.PADDING) / 2.0f) + iTranslateX, this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_relations_dec).getHeight() / 2 + iTranslateY);
                CFG.fontMain.getData().setScale(0.8f);
                CFG.drawText(oSB, this.getText(), this.getPosX() + (this.getTextPos() < 0 ? this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_relations_dec).getWidth() + (float)CFG.PADDING) / 2.0f) + ImageManager.getImage(Images.diplo_relations_dec).getWidth() + CFG.PADDING : this.getTextPos()) + iTranslateX, this.getPosY() + this.getHeight() / 2 - (int)((float)this.getTextHeight() * 0.8f / 2.0f) + iTranslateY, this.getColor(isActive));
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("SendAnInsult"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
    ...
    protected final void actionElement(int iID) {
        if (iID == this.getMenuElementsSize() - 1) {
            DiplomacyManager.decreaseRelation(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID, this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent());
