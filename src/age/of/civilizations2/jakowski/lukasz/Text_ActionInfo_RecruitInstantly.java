...
class Text_ActionInfo_RecruitInstantly extends Text_ActionInfo {
   protected Text_ActionInfo_RecruitInstantly(String sText, int iPosX, int iPosY) {
   ...
   protected void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
      ...
      ImageManager.getImage(Images.diplo_army)
         .draw(
            ...
            this.getPosY()
               + (int)(((float)this.getHeight() - (float)ImageManager.getImage(Images.diplo_army).getHeight() * this.getImageScale()) / 2.0F)
               - ImageManager.getImage(Images.flag_rect).getHeight()
               + iTranslateY,
            (int)((float)ImageManager.getImage(Images.diplo_army).getWidth() * this.getImageScale()),
            (int)((float)ImageManager.getImage(Images.diplo_army).getHeight() * this.getImageScale())
         );
      CFG.fontMain.getData().setScale(0.8F);
      CFG.drawTextWithShadow(
         oSB,
         this.sText,
         this.getPosX() + (int)((float)ImageManager.getImage(Images.diplo_army).getWidth() * this.getImageScale()) + CFG.PADDING * 2 + iTranslateX,
   ...
   private final float getImageScale() {
      return (float)CFG.TEXT_HEIGHT / (float)ImageManager.getImage(Images.diplo_army).getHeight();
   }

   @Override
   protected int getWidth() {
      return super.getWidth() + (int)((float)ImageManager.getImage(Images.diplo_army).getWidth() * this.getImageScale()) + CFG.PADDING * 2;
   }
