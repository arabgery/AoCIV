...
extends SliderMenu {
    ...
    protected Menu_InGame_CityFounded() {
    ...
    protected Menu_InGame_CityFounded(int nProvinceID, int nCivID) {
        ArrayList<MenuElement> menuElements = new ArrayList<MenuElement>();
        menuElements.add(new Button_Transparent(0, 0, 1, 1, false));
        this.initMenu(null, 0, 0, CFG.GAME_WIDTH, CFG.GAME_HEIGHT, menuElements);
        this.iCivA = nCivID;
        this.sWar = CFG.game.getProvince(nProvinceID).getCitiesSize() > 0 ? CFG.langManager.get("CityHasBeenFounded", CFG.game.getProvince(nProvinceID).getCity(0).getCityName()) : CFG.langManager.get("CityHasBeenFounded", CFG.game.getProvince(nProvinceID).getName());
