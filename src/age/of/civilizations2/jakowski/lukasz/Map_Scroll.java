...
class Map_Scroll {
   ...
   private Map_Scroll.BackgroundAnimation backgroundAnimation = new Map_Scroll.BackgroundAnimation() {
      @Override
      public void updateBackgroundAnimation() {
      }
   };
   private Map_Scroll.ReverseDirection reverseDirectionX = null;
   private Map_Scroll.ReverseDirection reverseDirectionY = null;

   protected final void updateEnableBackroundAnimation() {
      this.enableBackgroundAnimation = !CFG.menuManager.getInGameView()
      ...
      if (this.enableBackgroundAnimation) {
         if (CFG.menuManager.getInNewGamePlayers()) {
            this.backgroundAnimation = new Map_Scroll.BackgroundAnimation() {
               @Override
               public void updateBackgroundAnimation() {
            ...
            this.backgroundAnimation = new Map_Scroll.BackgroundAnimation() {
               ...
               public void updateBackgroundAnimation() {
                  if (!CFG.map.getMapTouchManager().getActionMap() && Map_Scroll.this.moveMapTime <= System.currentTimeMillis() - 85L) {
                     CFG.map
                        .getMapCoordinates()
                        .setNewPosX(CFG.map.getMapCoordinates().getPosX() + (int)((float)(Map_Scroll.this.moveMapDirection ? 1 : -1) * CFG.DENSITY));
   ...
   protected static final float changeAnimationPos(int animationStepID, int nWidth) {
   ...
   protected final void setScrollEvent(int nProvinceID) {
      this.setScrollEvent_Pos(
         (int)(
            (float)(CFG.map.getMapCoordinates().getPosX() + CFG.game.getProvince(nProvinceID).getCenterX())
               - (float)CFG.GAME_WIDTH / CFG.map.getMapScale().getCurrentScale() / 2.0F
         ),
         (int)(
            (float)(CFG.map.getMapCoordinates().getPosY() + CFG.game.getProvince(nProvinceID).getCenterY())
               - (float)CFG.GAME_HEIGHT / CFG.map.getMapScale().getCurrentScale() / 2.0F
         )
      );
   }
   ...
   protected final void setScrollEvent_ToPosition(int nPosX, int nPosY) {
      this.setScrollEvent_Pos(
         (int)((float)(CFG.map.getMapCoordinates().getPosX() + nPosX) - (float)CFG.GAME_WIDTH / CFG.map.getMapScale().getCurrentScale() / 2.0F),
         (int)((float)(CFG.map.getMapCoordinates().getPosY() + nPosY) - (float)CFG.GAME_HEIGHT / CFG.map.getMapScale().getCurrentScale() / 2.0F)
      );
   }
   ...
   protected final void buildReverseDirectionX() {
      if (CFG.reverseDirectionX) {
         this.reverseDirectionX = new Map_Scroll.ReverseDirection() {
            @Override
   ...
   protected final void buildReverseDirectionY() {
      if (CFG.reverseDirectionY) {
         this.reverseDirectionY = new Map_Scroll.ReverseDirection() {
            @Override
