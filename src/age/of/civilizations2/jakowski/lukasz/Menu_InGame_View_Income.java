...
import age.of.civilizations2.jakowski.lukasz.Button_View_Income;
...
extends SliderMenu {
    ...
    protected Menu_InGame_View_Income() {
        int i;
        ...
        for (i = 0; i < CFG.game.getCiv(this.iCivID).getNumOfProvinces(); ++i) {
            ...
            tempIncome = (int)((float)tempIncome + CFG.game_NextTurnUpdate.getProvinceIncomeAndExpenses_Total(CFG.game.getCiv(this.iCivID).getProvinceID(i)));
        }
        while (tempProvs.size() > 0) {
            int tBest = 0;
            for (int i2 = 1; i2 < tempProvs.size(); ++i2) {
                if (!(CFG.game_NextTurnUpdate.getProvinceIncomeAndExpenses_Total((Integer)tempProvs.get(tBest)) < CFG.game_NextTurnUpdate.getProvinceIncomeAndExpenses_Total((Integer)tempProvs.get(i2)))) continue;
                tBest = i2;
            }
        ...
        if (tempProvincesSorted.size() > 0) {
            menuElements.add(new Button_NS_Population(new Color((float)CFG.game.getCiv(this.iCivID).getR() / 255.0f, (float)CFG.game.getCiv(this.iCivID).getG() / 255.0f, (float)CFG.game.getCiv(this.iCivID).getB() / 255.0f, 1.0f), CFG.game.getCiv(this.iCivID).getCivName(), this.iCivID, CFG.langManager.get("Income") + ": ", "" + (tempIncome > 0 ? "+" : "") + CFG.getNumberWithSpaces("" + tempIncome), Images.top_gold, tempIncome >= 0 ? CFG.COLOR_INGAME_GOLD : CFG.COLOR_TEXT_MODIFIER_NEGATIVE2, 0, tY, tempW){});
            tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
            for (i = 0; i < tempProvincesSorted.size(); ++i) {
                menuElements.add(new Button_View_Income(i, (CFG.game.getProvince((Integer)tempProvincesSorted.get(i)).getName().length() > 0 ? CFG.game.getProvince((Integer)tempProvincesSorted.get(i)).getName() : CFG.game.getCiv(this.iCivID).getCivName()) + ": ", (Integer)tempProvincesSorted.get(i), 0, tY, CFG.CIV_INFO_MENU_WIDTH, (CFG.SPECTATOR_MODE || CFG.game.isAlly(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iCivID)) && CFG.game.getCiv(this.iCivID).isInConstruction((Integer)tempProvincesSorted.get(i), ConstructionType.WORKSHOP) > 0){

                    @Override
                    protected void actionElement(int iID) {
                        if (CFG.gameAction.getActiveTurnState() == Game_Action.TurnStates.INPUT_ORDERS && !CFG.SPECTATOR_MODE && this.getCurrent() == CFG.game.getActiveProvinceID() && CFG.game.getProvince(this.getCurrent()).getCivID() == CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID() && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).isInConstruction(this.getCurrent(), ConstructionType.WORKSHOP) <= 0) {
        ...
        this.initMenu(new SliderMenuTitle(CFG.game.getCiv(this.iCivID).getCivName(), CFG.BUTTON_HEIGHT * 3 / 5, false, false){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ImageManager.getImage(Images.new_game_top_edge_title).draw2(oSB, Menu_InGame_View_Income.this.getPosX() + iTranslateX, Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(), Menu_InGame_View_Income.this.getWidth() + 2, this.getHeight(), true, false);
                ...
                ImageManager.getImage(Images.pix255_255_255).draw(oSB, Menu_InGame_View_Income.this.getPosX() + iTranslateX, Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(), Menu_InGame_View_Income.this.getWidth());
                oSB.setColor(new Color(CFG.COLOR_FLAG_FRAME.r, CFG.COLOR_FLAG_FRAME.g, CFG.COLOR_FLAG_FRAME.b, 0.4f));
                ImageManager.getImage(Images.line_32_off1).draw(oSB, Menu_InGame_View_Income.this.getPosX() + iTranslateX, Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight() - ImageManager.getImage(Images.line_32_off1).getHeight(), Menu_InGame_View_Income.this.getWidth(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.6f));
                ImageManager.getImage(Images.line_32_off1).draw(oSB, Menu_InGame_View_Income.this.getPosX() + iTranslateX, Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight() - ImageManager.getImage(Images.line_32_off1).getHeight() - 1, Menu_InGame_View_Income.this.getWidth(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.8f));
                ImageManager.getImage(Images.slider_gradient).draw(oSB, Menu_InGame_View_Income.this.getPosX() + iTranslateX, Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight() - ImageManager.getImage(Images.slider_gradient).getHeight(), Menu_InGame_View_Income.this.getWidth() / 4, 1);
                ImageManager.getImage(Images.slider_gradient).draw(oSB, Menu_InGame_View_Income.this.getPosX() + Menu_InGame_View_Income.this.getWidth() - Menu_InGame_View_Income.this.getWidth() / 4 + iTranslateX, Menu_InGame_View_Income.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight() - ImageManager.getImage(Images.slider_gradient).getHeight(), Menu_InGame_View_Income.this.getWidth() / 4, 1, true, false);
                oSB.setColor(Color.WHITE);
                try {
                    CFG.game.getCiv(Menu_InGame_View_Income.this.iCivID).getFlag().draw(oSB, nPosX + CFG.PADDING * 2 + iTranslateX, nPosY - this.getHeight() + this.getHeight() / 2 + 1 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.game.getCiv(Menu_InGame_View_Income.this.iCivID).getFlag().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
