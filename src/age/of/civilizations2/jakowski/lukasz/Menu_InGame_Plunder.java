...
import age.of.civilizations2.jakowski.lukasz.Button_Diplomacy_Plunder;
...
class Menu_InGame_Plunder
extends SliderMenu {
    protected static int iProvinceID = -1;

    protected Menu_InGame_Plunder() {
    ...
    protected Menu_InGame_Plunder(final int nProvinceID) {
        ...
        for (int i = 0; i < CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoveUnitsPlunderSize(); ++i) {
            if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoveUnits_Plunder(i).getFromProvinceID() != nProvinceID) continue;
            currArmyPlunder = CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoveUnits_Plunder(i).getNumOfUnits();
            break;
        }
        menuElements.add(new Slider_FlagAction_Clear(CFG.langManager.get("Army"), CFG.PADDING * 2, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4, 0, CFG.game.getProvince(nProvinceID).getArmyCivID(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) + currArmyPlunder, currArmyPlunder == 0 ? CFG.game.getProvince(nProvinceID).getArmyCivID(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) : currArmyPlunder, 0.65f){
            ...
            @Override
            protected void actionElement(int iID) {
                Menu_InGame_Plunder.this.getMenuElement(0).setCurrent((int)(DiplomacyManager.plunderEfficiency(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), nProvinceID, this.getCurrent()) * 100.0f));
            }
        });
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Flag(CFG.game.getProvince(iProvinceID).getTrueOwnerOfProvince()));
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("" + Menu_InGame_Plunder.this.getMenuElement(1).getCurrent(), CFG.COLOR_BUTTON_GAME_TEXT_ACTIVE));
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("+" + DiplomacyManager.plunderTreasuryIncome(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), iProvinceID, Menu_InGame_Plunder.this.getMenuElement(1).getCurrent()), CFG.COLOR_INGAME_GOLD));
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("-" + (float)((int)(DiplomacyManager.plunder_LossesEconomy_Perc(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), iProvinceID, Menu_InGame_Plunder.this.getMenuElement(1).getCurrent()) * 10000.0f)) / 100.0f + "%", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Text("-" + (float)((int)(DiplomacyManager.plunder_Happiness(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), iProvinceID, Menu_InGame_Plunder.this.getMenuElement(1).getCurrent()) * 10000.0f)) / 100.0f + "%", CFG.COLOR_TEXT_MODIFIER_NEGATIVE2));
            ...
            protected void drawText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                ImageManager.getImage(Images.diplo_plunder).draw(oSB, this.getPosX() + this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_plunder).getWidth() + (float)CFG.PADDING) / 2.0f) + iTranslateX, this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.diplo_plunder).getHeight() / 2 + iTranslateY);
                CFG.fontMain.getData().setScale(0.8f);
                CFG.drawText(oSB, this.getText(), this.getPosX() + (this.getTextPos() < 0 ? this.getWidth() / 2 - (int)(((float)this.getTextWidth() * 0.8f + (float)ImageManager.getImage(Images.diplo_plunder).getWidth() + (float)CFG.PADDING) / 2.0f) + ImageManager.getImage(Images.diplo_plunder).getWidth() + CFG.PADDING : this.getTextPos()) + iTranslateX, this.getPosY() + this.getHeight() / 2 - (int)((float)this.getTextHeight() * 0.8f / 2.0f) + iTranslateY, this.getColor(isActive));
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Plunder") + (CFG.game.getProvince(nProvinceID).getName().length() > 0 ? ": " + CFG.game.getProvince(nProvinceID).getName() : ""), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.game.getCiv(CFG.game.getProvince(iProvinceID).getTrueOwnerOfProvince()).getFlag().draw(oSB, nPosX + CFG.PADDING * 2 + iTranslateX, nPosY - this.getHeight() + this.getHeight() / 2 + 1 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.game.getCiv(CFG.game.getProvince(iProvinceID).getTrueOwnerOfProvince()).getFlag().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
        ...
        this.getMenuElement(0).setCurrent((int)(DiplomacyManager.plunderEfficiency(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), nProvinceID, this.getMenuElement(1).getCurrent()) * 100.0f));
