...
class Menu_Continents_CreateNewContinent
extends SliderMenu {
    private String sName;
    private int iNameWidth;

    protected Menu_Continents_CreateNewContinent() {
        ...
        menuElements.add(new Button_Game(null, -1, CFG.PADDING, CFG.BUTTON_HEIGHT + CFG.PADDING * 3, CFG.BUTTON_WIDTH * 2){
            ...
            @Override
            protected Color getColor(boolean isActive) {
                return isActive ? new Color(CFG.editor_Continent_GameData.getR(), CFG.editor_Continent_GameData.getG(), CFG.editor_Continent_GameData.getB(), 1.0f) : (this.getClickable() ? new Color(0.38f, 0.38f, 0.38f, 1.0f) : new Color(0.49f, 0.49f, 0.49f, 0.5f));
            }
        });
