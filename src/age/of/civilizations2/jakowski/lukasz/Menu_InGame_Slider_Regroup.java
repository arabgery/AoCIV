...
class Menu_InGame_Slider_Regroup
extends SliderMenu {
    protected Menu_InGame_Slider_Regroup() {
        ...
        block9: {
            try {
                if (CFG.game.currentRegroupArmy.getRouteSize() == 1) {
                    CFG.gameAction.moveArmy(CFG.game.getActiveProvinceID(), CFG.chosenProvinceID, this.getMenuElement(2).getCurrent(), CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), true, true);
                } else if (CFG.gameAction.moveArmy(CFG.game.getActiveProvinceID(), CFG.game.currentRegroupArmy.getRoute(0), this.getMenuElement(2).getCurrent(), CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), true, true)) {
