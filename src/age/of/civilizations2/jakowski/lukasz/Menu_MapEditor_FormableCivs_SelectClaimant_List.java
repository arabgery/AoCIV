...
class Menu_MapEditor_FormableCivs_SelectClaimant_List
extends SliderMenu {
    ...
    protected Menu_MapEditor_FormableCivs_SelectClaimant_List() {
        ...
        if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
            ...
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0 || CFG.isInFormableCivs(tagsSPLITED[i])) continue;
                lTempNames.add(CFG.langManager.getCiv(tagsSPLITED[i]));
                lTempTags.add(tagsSPLITED[i]);
            }
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED_ED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0 || CFG.isInFormableCivs(tagsSPLITED_ED[i])) continue;
            ...
            int iSize = tagsSPLITED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED[i]).charAt(0) != CFG.chosen_AlphabetCharachter.charAt(0) || CFG.isInFormableCivs(tagsSPLITED[i])) continue;
                lTempNames.add(CFG.langManager.getCiv(tagsSPLITED[i]));
                lTempTags.add(tagsSPLITED[i]);
            }
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.langManager.getCiv(tagsSPLITED_ED[i]).charAt(0) != CFG.chosen_AlphabetCharachter.charAt(0) || CFG.isInFormableCivs(tagsSPLITED_ED[i])) continue;
                lTempNames.add(CFG.langManager.getCiv(tagsSPLITED_ED[i]));
                lTempTags.add(tagsSPLITED_ED[i]);
            }
            int nPosY = 0;
