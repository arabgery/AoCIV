...
class Pallet_Manager {
   ...
   protected final void updatePalletsOfCivsColorsTags() {
      try {
         ...
         for(int i = 0; i < this.iNumOfPallets; ++i) {
            ...
            List<Color> tempSampleColors = new ArrayList<>();
            ...
            for(int j = 0; j < 10 && j < tagsSPLITED2.length; ++j) {
               try {
                  try {
                     ...
                     if (this.isInternal.get(i)) {
                        tempColor = (PalletOfCivsColors_Civ_GameData)CFG.deserialize(
                           Gdx.files.internal("game/pallets_of_civs_colors/" + (String)this.lPalletsTags.get(i) + "/" + tagsSPLITED2[j]).readBytes()
                        );
                     } else {
                        tempColor = (PalletOfCivsColors_Civ_GameData)CFG.deserialize(
                           Gdx.files.local("game/pallets_of_civs_colors/" + (String)this.lPalletsTags.get(i) + "/" + tagsSPLITED2[j]).readBytes()
                        );
                     }
   ...
   protected final void loadCivilizationsPaletteOfColors(int nPaletteID) {
      if (nPaletteID == 0) {
         ...
         for(int i = 1; i < CFG.game.getCivsSize(); ++i) {
            ...
            try {
               if (this.isInternal.get(nPaletteID - 1)) {
                  file = Gdx.files
                     .internal("game/pallets_of_civs_colors/" + (String)this.lPalletsTags.get(nPaletteID - 1) + "/" + CFG.game.getCiv(i).getCivTag());
               } else {
                  file = Gdx.files
                     .local("game/pallets_of_civs_colors/" + (String)this.lPalletsTags.get(nPaletteID - 1) + "/" + CFG.game.getCiv(i).getCivTag());
               }
   ...
   protected final void loadCivilizationPalletColor(int nPaletteID, int nCivID) {
      try {
         ...
         if (this.isInternal.get(nPaletteID - 1)) {
            file = Gdx.files
               .internal("game/pallets_of_civs_colors/" + (String)this.lPalletsTags.get(nPaletteID - 1) + "/" + CFG.game.getCiv(nCivID).getCivTag());
         } else {
   ...
   protected final void drawSampleColors(SpriteBatch oSB, int nPosX, int nPosY, int nWidth, int nHeight, int nPalletID, boolean isActive) {
      try {
         ...
         for(int iSize = this.lSampleColors.get(nPalletID).size(); i < iSize; ++i) {
